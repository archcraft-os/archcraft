...
iso_application="A Minimalistic Linux Distribution, Focused On Aesthetics & Based On Arch Linux"
...
make_basefs() {
    if [ -n "${verbose}" ]; then
        ...
        mkarchiso -v -w "${work_dir}/x86_64" -C "${work_dir}/pacman.conf" -D "${install_dir}" \
            -p "haveged intel-ucode amd-ucode mkinitcpio-nfs-utils nbd zsh efitools" install
    else
